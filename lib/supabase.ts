	...
	try {
		...
		if (testimonialsResult.data) {
			...
			config.testimonials = {
				...
				subtitle:
					testimonialsSection.subtitle || "Success stories from our community",
...
const ensureHomePageExists = async (
	supabase: TypedSupabaseClient
): Promise<string> => {
	try {
...
export const updateSectionContent = async (
	...
	try {
		...
		console.log(
			`🔍 Checking for existing page section: page_id=${homePageId}, section_id=${section.id}`
		);
		...
		if (existingPageSection) {
			// Update existing
			console.log(
				`📝 Updating existing page section: ${existingPageSection.id}`
			);
			const { data, error } = await supabase
				.from("page_sections")
				.update({
	...
	try {
		const results = await Promise.allSettled(
			testimonials.map((testimonial) =>
				upsertTestimonial(supabase, testimonial)
			)
		);
		...
		console.warn(
			"Bulk testimonial update failed:",
			err instanceof Error ? err.message : "Unknown error"
		);
		throw err;
	}
...
export const setGlobalSettings = async (
	...
	try {
		...
		console.warn(
			"Bulk global settings update failed:",
			err instanceof Error ? err.message : "Unknown error"
		);
		throw err;
	}
