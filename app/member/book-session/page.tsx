...
// 1. Add interfaces for session and package data
...
export default function BookSessionPage() {
	...
	const [weeklyAvailableSessions, setWeeklyAvailableSessions] = useState<any>(
		{}
	);
				...
				packagesData.forEach((pkg: any) => {
					...
					sessionTypes.forEach((sessionType) => {
						transformedPackages[sessionType] = {
							...
							total: pkg.packages?.session_count || 0,
							expiry: pkg.end_date
	...
	const handleBookSession = async (session) => {
        ...
        `
			...
			for (const pkg of memberPackageData) {
				...
				if (
					packageInfo.packageType === pkgType ||
			...
			Object.keys(updatedWeeklySessions).forEach((weekKey) => {
				updatedWeeklySessions[weekKey] = updatedWeeklySessions[weekKey].map(
					(s) => {
						if (s.id === session.id) {
							return {
								...s,
	...
	const handleCancelSession = async (session) => {
		...
		try {
			...
			toast({
				...
				description:
					"Your session has been cancelled and reimbursed to your package.",
			});
		} catch (error) {
		...
		<div className="container mx-auto py-6 space-y-6">
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Available sessions in your current packages
					</CardDescription>
				</CardHeader>
				<CardContent>
				...
				<TabsContent value="booked">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								All your booked, completed, and cancelled sessions
							</CardDescription>
						</CardHeader>
						<CardContent>
