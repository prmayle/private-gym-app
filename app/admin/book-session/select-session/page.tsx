...
export default function SelectSessionPage() {
  ...
  const filterSessions = () => {
    ...
    const memberPackageTypes = member.packages
      ...
      .map((pkg: any) => (typeof pkg === "object" && pkg.type ? pkg.type : pkg))

    // Only show sessions that match member's available packages
    filtered = filtered.filter((session) => memberPackageTypes.includes(session.type))
    ...
    if (trainerFilter !== "all") {
      filtered = filtered.filter((session) => session.trainer === trainerFilter)
    }
  ...
  const canBookSession = (session: Session) => {
    ...
    return hasAvailablePackage && session.status === "Available" && session.capacity.booked < session.capacity.total
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center">
        ...
        <div>
          <h1 className="text-2xl font-bold">Select Session for {member.name}</h1>
          <p className="text-muted-foreground">Step 2: Choose an available session to book</p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Sessions that match your member's available packages</CardDescription>
        </CardHeader>
        <CardContent>
          {loading && (
          ...
          {!loading && filteredSessions.length === 0 && (
            <div className="text-center py-8">
              ...
              <p className="text-muted-foreground mb-2">No available sessions found for this member's packages.</p>
              <Button variant="outline" onClick={() => router.push("/admin/book-session")}>
                        ...
                        <TableCell>
                          {packageInfo ? (
                            <div className="flex items-center gap-1">
                              ...
                              <span className="text-sm text-green-600">{packageInfo.remaining || 0} sessions left</span>
                            </div>
                          ) : (
                            <span className="text-sm text-red-600">No matching package</span>
                          )}
                        </TableCell>
