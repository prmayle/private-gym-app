...
const mockTemplates = [
  ...
  },
  {
    ...
    content: "Hi {memberName}, we have an important announcement: {announcement}. Thank you for your attention.",
  },
]
...
export default function NewNotificationPage() {
  ...
  const handleSendNotification = async () => {
    ...
    if (!formData.sendToAll && selectedMembers.length === 0) {
      toast({
        title: "No Recipients",
        description: "Please select at least one member or choose 'Send to All'.",
        variant: "destructive",
      })
      return
    }
    ...
    try {
      ...
      const recipients = formData.sendToAll
        ? mockMembers
        : mockMembers.filter((member) => selectedMembers.includes(member.id))
      ...
      const newNotification = {
        ...
        type: selectedTemplate ? mockTemplates.find((t) => t.id === selectedTemplate)?.name || "Custom" : "Custom",
        template: selectedTemplate ? mockTemplates.find((t) => t.id === selectedTemplate)?.name : null,
      ...
      const existingSentNotifications = JSON.parse(localStorage.getItem("sent-notifications") || "[]")
      existingSentNotifications.unshift(newNotification) // Add to beginning
      localStorage.setItem("sent-notifications", JSON.stringify(existingSentNotifications))

      // Deliver to member notifications
      const existingMemberNotifications = JSON.parse(localStorage.getItem("member-notifications") || "[]")
      recipients.forEach((recipient) => {
        const memberNotification = {
          id: `${newNotification.id}-${recipient.id}`,
        ...
        existingMemberNotifications.unshift(memberNotification)
      })
      localStorage.setItem("member-notifications", JSON.stringify(existingMemberNotifications))

      const recipientCount = recipients.length

      toast({
        title: "Notification Sent",
        description: `Notification sent successfully to ${recipientCount} member(s).`,
      })
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Create and send notifications to your members</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  You can use placeholders like {"{memberName}"}, {"{sessionType}"}, {"{date}"}, {"{time}"}, etc.
                </p>
              </div>
              ...
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => setFormData({ ...formData, sendToAll: checked })}
                />
              ...
              {formData.sendToAll && (
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-muted-foreground">
                    This notification will be sent to all {mockMembers.length} members.
                  </p>
                </div>
              )}
            </CardContent>
